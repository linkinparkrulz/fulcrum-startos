...
class FileHelper {
    ...
    async *readWatch(effects, map, eq) {
        let res;
    ...
    async readWaitFor(effects, pred, map) {
        while (effects.isInContext) {
            if (await exists(this.path)) {
                const ctrl = new AbortController();
                const watch = fs.watch(this.path, {
    ...
    read(map, eq) {
        map = map ?? ((a) => a);
        eq = eq ?? ((left, right) => !(0, util_1.partialDiff)(left, right));
        return {
            once: () => this.readOnce(map),
            const: (effects) => this.readConst(effects, map, eq),
            watch: (effects) => this.readWatch(effects, map, eq),
    ...
    async write(effects, data, options = {}) {
        ...
        if (!options.allowWriteAfterConst &&
            effects.constRetry &&
